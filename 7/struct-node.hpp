...
protected:
    ...
    explicit Node(std::string name, size_t file_size) : m_name(std::move(name)), m_size(file_size) {}

    explicit Node(std::string name, Node::Type type) : m_name(std::move(name)), m_type(type) {}
};
